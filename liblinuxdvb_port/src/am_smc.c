...
static inline int smc_get_openned_dev(int dev_no, AM_SMC_Device_t **dev)
{
	AM_TRY(smc_get_dev(dev_no, dev));
	
	if(!(*dev)->openned)
	{
		CA_DEBUG(1, "smartcard device %d has not been openned", dev_no);
...
/**\brief 从智能卡读取数据*/
static int smc_read(AM_SMC_Device_t *dev, uint8_t *buf, int len, int *act_len, int timeout)
{
	uint8_t *ptr = buf;
	int left = len;
	...
	while (left)
	{
		...
		if(timeout>=0)
		{
			...
			if(left && diff>=0)
			{
				CA_DEBUG(1, "read %d bytes timeout", len);
	...
	if(act_len)
		*act_len = cnt;
     CA_DEBUG(1, "smc_read ret is:%d", ret);
	return ret;
}

/**\brief 向智能卡发送数据*/
static int smc_write(AM_SMC_Device_t *dev, const uint8_t *buf, int len, int *act_len, int timeout)
{
	...
	while (left)
	{
		...
		if(timeout>=0)
		{
			...
			if(left && diff>=0)
			{
				CA_DEBUG(1, "write %d bytes timeout", len);
				ret = AM_SMC_ERR_TIMEOUT;
				break;
			}
		}
 ...
 *   - 其他值 错误代码(见am_smc.h)
 */
int AM_SMC_Open(int dev_no, const AM_SMC_OpenPara_t *para)
{
	AM_SMC_Device_t *dev;
	int ret = 0;
	...
	if(dev->openned)
	{
		CA_DEBUG(1, "smartcard device %d has already been openned", dev_no);
		ret = AM_SMC_ERR_BUSY;
		goto final;
	}
	...
	if(dev->enable_thread)
	{
		...
		{
			CA_DEBUG(1, "cannot create the status monitor thread");
 ...
 *   - 其他值 错误代码(见am_smc.h)
 */
int AM_SMC_GetCardStatus(int dev_no, AM_SMC_CardStatus_t *status)
{
	AM_SMC_Device_t *dev;
	int ret = 0;
	...
	{
		CA_DEBUG(1, "driver do not support get_status");
 ...
 *   - 其他值 错误代码(见am_smc.h)
 */
int AM_SMC_Reset(int dev_no, uint8_t *atr, int *len)
{
	AM_SMC_Device_t *dev;
	int ret = 0;
	...
	if(atr && len)
	{
		...
		{
			CA_DEBUG(1, "ATR buffer < %d", AM_SMC_MAX_ATR_LEN);
	...
	if(!dev->drv->reset)
	{
		CA_DEBUG(1, "driver do not support reset");
		ret = AM_SMC_ERR_NOT_SUPPORTED;
	}
 ...
 *   - 其他值 错误代码(见am_smc.h)
 */
int AM_SMC_Read(int dev_no, uint8_t *data, int len, int timeout)
{
	AM_SMC_Device_t *dev;
	int ret = 0;
 ...
 *   - 其他值 错误代码(见am_smc.h)
 */
int AM_SMC_Write(int dev_no, const uint8_t *data, int len, int timeout)
{
	AM_SMC_Device_t *dev;
	int ret = 0;
 ...
 *   - 其他值 错误代码(见am_smc.h)
 */
int AM_SMC_TransferT0(int dev_no, const uint8_t *send, int slen, uint8_t *recv, int *rlen)
{
	AM_SMC_Device_t *dev;
	int ret = 0;
	uint8_t byte;
	...
	while(1)
	{
		...
		CA_DEBUG(1, "...INS byte is 0x%x, rxlen is:%d.\n", byte, rxlen);
		if(byte ==0x60)
		{
			continue;
		}
 ...
 *   - 其他值 错误代码(见am_smc.h)
 */
int AM_SMC_TransferT1(int dev_no, const uint8_t *send, int slen, uint8_t *recv, int *rlen)
{
	AM_SMC_Device_t *dev;
	int ret = 0;
 ...
 *   - 其他值 错误代码(见am_smc.h)
 */
int AM_SMC_TransferT14(int dev_no, const uint8_t *send, int slen, uint8_t *recv, int *rlen)
{
	AM_SMC_Device_t *dev;
	int ret = 0;
 ...
 *   - 其他值 错误代码(见am_smc.h)
 */
int AM_SMC_GetCallback(int dev_no, AM_SMC_StatusCb_t *cb, void **data)
{
	AM_SMC_Device_t *dev;
	int ret = 0;
 ...
 *   - 其他值 错误代码(见am_smc.h)
 */
int AM_SMC_SetCallback(int dev_no, AM_SMC_StatusCb_t cb, void *data)
{
	AM_SMC_Device_t *dev;
	int ret = 0;
...
int AM_SMC_Active(int dev_no)
{
	...
	if(!dev->drv->active)
	{
		CA_DEBUG(1, "driver do not support active");
		ret = AM_SMC_ERR_NOT_SUPPORTED;
	}
 ...
 *   - 其他值 错误代码(见am_smc.h)
 */
int AM_SMC_PraseATR(int dev_no,unsigned char* atr,int len)
{
	AM_SMC_Device_t *dev;
	int ret = 0;
 ...
 *   - 其他值 错误代码(见am_smc.h)
 */
int AM_SMC_Deactive(int dev_no)
{
	AM_SMC_Device_t *dev;
	int ret = 0;
	...
	if(!dev->drv->deactive)
	{
		CA_DEBUG(1, "driver do not support deactive");
		ret = AM_SMC_ERR_NOT_SUPPORTED;
	}
 ...
 *   - 其他值 错误代码(见am_smc.h)
 */
int AM_SMC_GetParam(int dev_no, AM_SMC_Param_t *para)
{
	AM_SMC_Device_t *dev;
	int ret = 0;
	...
	{
		CA_DEBUG(1, "driver do not support get_param");
		ret = AM_SMC_ERR_NOT_SUPPORTED;
	}
 ...
 *   - 其他值 错误代码(见am_smc.h)
 */
int AM_SMC_SetParam(int dev_no, const AM_SMC_Param_t *para)
{
	AM_SMC_Device_t *dev;
	int ret = 0;
	...
	{
		CA_DEBUG(1, "driver do not support set_param");
		ret = AM_SMC_ERR_NOT_SUPPORTED;
	}
