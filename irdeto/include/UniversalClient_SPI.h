...
/**
*Size of CCC Chip Configuration
*/
#define UC_CCC_CHIP_CONFIGURATION_SIZE 16
...
typedef struct _uc_chip_configuration_response 
{
   ...
   /**
    * 128-bit ChipConfiguration
    */
    uc_byte chipConfiguration[UC_CCC_CHIP_CONFIGURATION_SIZE];    
 ...
 *
 * Implementation is required for proper functioning of the CCA Agent.
 */
...
typedef enum {
    ...
    /** 
     * load Transformation data container (TDC) structure by IFCP command, where TDC is protected by TASK.
     * 
 ...
 *
 * Implementation is required for proper functioning of SMP with the CCA Agent.
 */
 ...
 *
 * \note This SPI is required for IP implementations. \n
 *
 * @retval
 *     ::UC_ERROR_SUCCESS
 * @retval
 ...
 * Devices that are for demo purpose or for test purpose may NOT have private data at first. In such cases, this SPI implementation
 * can set the pData->length to ZERO and return UC_ERROR_SUCCESS directly.
 *
 ...
 * Secure chips must be able to generate a chip configuration and a MAC using the standard CBC-MAC scheme with AES-128 block cipher.
 ...
 *
 * This method is called when the received stream contains Macrovision mode info or submitted Session Metadata contains Macrovision mode.
 *
 * @param[in] streamHandle Stream handle originally returned from the stream's ::UniversalClientSPI_Stream_Open method.
 ...
 *
 * This method is called when the received ECM contains the CopyControl/CI Plus info or the submitted Session Metadata contains the CopyControl/CI Plus info.
 *
 * @param[in] streamHandle Stream handle originally returned from the stream's ::UniversalClientSPI_Stream_Open method.
 ...
 * @retva
 *    ::UC_ERROR_SCOT_NOT_SUPPORTED_BY_CHIPSET
 * @retval
 *     Other The method may return any other errors listed in \ref result "Result Code List". 
 */
 ...
 *
 * Implementation is optional but is required for proper functioning of SMP with the CCA Agent.
 * 
