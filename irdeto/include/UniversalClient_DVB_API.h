...
/**
 * Send ECM data to the Cloaked CA Agent.
 *
 * Currently, this function is only used for VOD service that was previously configured using ::UC_TLV_TAG_FOR_VOD tag.
 * \note Refer to \subpage vod_playback_ecmapi_flow.
 *
 * @param[in] serviceHandle Service handle previously opened by a call to ::UniversalClient_OpenService.
 * @param[in] ecmPid ECM PID for ECM data.
 * @param[in] pEcm Buffer containing the ECM data. The uc_buffer_st::bytes member must point to a buffer containing the ECM data, 
 *    and the uc_buffer_st::length must be set to the number of bytes in the buffer. When the ECM is constituted of multiple sections, 
 *    all sections of the ECM need to be delivered in one call. The buffer shall contain these ECM sections concatenated one after the other.
 * @param[in] userData User data. The caller can use this data to indicate anything in the caller's area. 
 *
 * @retval 
 *    ::UC_ERROR_SUCCESS
 * @retval 
 ...
 *     Other The method may return any other errors listed in \ref result "Result Code List". 
 */
uc_result UniversalClient_DVB_ProcessECM(
    uc_service_handle  serviceHandle,
    uc_uint16  ecmPid, 
    const uc_buffer_st *pEcm, 
    uc_uint32 userData);
