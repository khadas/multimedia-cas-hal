...
/**
 * Maximum count of ECM PIDs in a VOD service
 */
#define UC_MAX_VOD_ECM_PID_COUNT   64
/** @} */ /* End of groups */
...
/**
 * Service Status Screen Text: No original product entitlement was used during recording to perform PVR playback.
 *
 * No original product entitlement was used during recording. This message is sent when a PVR playback request is rejected by the client because the client did not find
 * the original product entitlement, which was used during recording to perform playback.
 *
 */
#define ERR_MSG_E136        "E136-0  "

/**
 * Service Status Screen Text: Product blackout during PVR playback.
 *
 * Product blackout for recorded content during PVR playback. This message is sent when a PVR playback request is rejected by the client 
 * because a product blackout filter imposed in the recorded content.
 *
 */
#define ERR_MSG_E137        "E137-0  "

/**
 * Service Status Screen Text: PVR sharing is prohibited due to domain ID mismatch.
 *
 * PVR Domain mismatch. This message is sent when Cloaked CA does not have a valid domain, the domain ID is mismatched, or Cloaked CA cannot find a correct MSK. 
 * That means sharing playback is prohibited. This error code is only available for Home Network PVR.
 *
 */
...
/**
 * Service Status Screen Text: The platform does not support Secure Client Output Transformation (SCOT).
 *
 * This message is sent when an ECM is rejected by the client because the platform does not support SCOT.
 */
#define ERR_MSG_E171        "E171-0  "
...
/**
 * Tag: Set the service to VOD Service.
 *
 * This tag is used to set the service to VOD Service.
 * If the application uses VOD, it shall open a service and set the service with this TLV.
 * The TLV is in the following format:
 \code
 ...
 TLV_VOD()
 {
     ...
     Tag                1  byte             UC_TLV_TAG_FOR_VOD
     Length             2  bytes            0
 }
 \endcode

 \note Refer to \subpage vod_playback_ecmapi_flow.

 */
#define UC_TLV_TAG_FOR_VOD   (0x08)

/**
 * Tag: Set the service to stop the VOD Service.
 *
 * This tag is used to send a stop VOD service request to the Cloaked CA Agent.
 * If the application is trying to stop a VOD service , it should make a request to the Cloaked CA Agent via this TLV.
 * If this TLV is processed successfully, Cloaked CA Agent will stop the VOD service.
 * The TLV is in the following format:
 \code
 ...
 TLV_VOD()
 {
     ...
     Tag                1  byte             UC_TLV_TAG_FOR_STOP_VOD
     Length             2  bytes            0
 }
 \endcode
 ...
 */
#define UC_TLV_TAG_FOR_STOP_VOD   (0x09)
...
typedef enum
{
   ...
   UC_GLOBAL_NOTIFY_HGPC_EXPIRY               = 0x00000020,
	...
	/**
	 * (0x00000021) The list of active operators.
	 *
	 * This message provides a list of all active (paired) operators. 
	 *
	 * This message will be sent out when the Cloaked CA Agent starts and when changes to pairing occur. 
	 *
	 *
	 * For messages with this type, the \a lpVoid argument points to a uc_buffer_st with the uc_buffer_st::length is the number of active operators and 
	 * the uc_buffer_st::byte is a pointer to a list of active operator IDs as uc_uint32s.
     * Middleware need not free this pointer.
	 */
	UC_GLOBAL_NOTIFY_ACTIVE_OPERATORS		   = 0x00000021    
 ...
 * SCOT TDC max number
 */
#define UC_TESTING_SCOT_TDC_MAX  3
 ...
 * SCOT  info structure 
 * 
 * The structure is part of the ::uc_testing_key_info structure and is passed to ::UniversalClient_SetTestingKeyForMultipleMode.
 */
typedef struct _uc_testing_scot_info
{

    /**
     * The key material of TDC prime. If there is no invalid TDC data for pTDC_Prime[i](i < UC_TESTING_SCOT_TDC_MAX), please set NULL(0) to pTDC_Prime[i].
      */ 
    uc_buffer_st *pTDC_Prime[UC_TESTING_SCOT_TDC_MAX]; 
    
    /**
     * The key material of SCOT key level-1.
      */
    uc_buffer_st *pTASK_Prime;
    
    /**
     * The key material of SCOT key level-2.
     * It is null(0) if secure chipset pairing mode is UC_SECURECHIP_PAIRING_MODE_MSR. 
     * It is valid if secure chipset pairing mode is UC_SECURECHIP_PAIRING_MODE_IFCP
     */
    uc_buffer_st *pSCOT_L2;    

    
}uc_testing_scot_info;
...
typedef struct _uc_testing_key_info
{
     ...
     * For SCOT, it is &lt;CW_ODD_SCOT&gt; in STB file.
     */
     ...
     * For OTT key ladder, it is &lt;OTTK0&gt; in STB file.
     * For SCOT, it is &lt;CW_EVEN_SCOT&gt; in STB file.
     */
    ...
    /**
     * Indicates SCOT mode. If it's ture, descramblingKeyProtection should be set to "UC_DK_PROTECTION_AES". 
     */
    uc_bool isSCOTMode;
    
    /**
     * The SCOT information.
     */     
    uc_testing_scot_info *pSCOTInfo;
